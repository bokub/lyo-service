...
async function compile(module) {
	const installOptions = {name: module, path: join(__dirname, '..', 'tmp')};
...
function prepare(options, pkg) {
	pkg.file = path.basename(options.output);
	pkg.exportedName = path.basename(options.name);
	pkg.publishName = '@lyo/' + pkg.name.replace('@', '').replace('/', '.');
	pkg.usage = usage.getModuleSignature({input: options.output, name: options.name});
	...
	fs.writeFileSync(path.dirname(options.output) + '/package.json', packageContent);
	fs.writeFileSync(path.dirname(options.output) + '/README.md', readmeContent);
